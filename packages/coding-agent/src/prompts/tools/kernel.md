...
- **Dead-ends by design:** `harness.recordEvaluation` denies in-session (evaluator-only); `harness.promote` returns "awaiting trusted verdict" for your proposals unless a trusted verdict exists. The TRUSTED path is the kernel gateway daemon (project-scoped ledger): the metaharness records benchmark verdicts via `harness.recordEvaluation` over the daemon RPC when a run completes with `harnessVersion` set — benchmark runs are the evaluator for `harness.promote`. `harness.void` retracts YOUR OWN junk/probe proposals (author-scoped; the baseline and active head are never voidable).
