...
- `kernel.routing.resolve({ role, taskComplexity?, … })` is ADVISORY (rule-based recommendation — the live session's actual model is decided by ModelControls, not this table). `kernel.routing.stats()` is read-only telemetry from the event log. `kernel.perf.profile({ limit? })` is the harness profiler: per-tool latency percentiles (p50/p90/p95/max) + output-bytes from tool.completed events — run it to find bottlenecks before optimizing (a tool with high p95 AND high outputBytes is both slow and cache-expensive). `routing.register`/`routing.record` were REMOVED (round-11 S1: dead surfaces — nothing read the registry, and the trajectory tap feeds the event log directly).
- `kernel.policy.authorize({ id, effect, resource, actor? })` → `{ allow, reason? }`. `kernel.security.profile({ actor? })` → tier + effective capabilities.
- `kernel.harness.hypothesis({ component, observation, hypothesis, prediction?, change?, evaluationSlice? })` → version (propose a harness change; MOST components are editable — only the 4 constitutional ones are refused. Proposing records a durable version, but there is NO wired trusted evaluator for RLM proposals, so promote returns "awaiting trusted verdict" indefinitely — treat hypothesis as telemetry, not a change you can land). `kernel.harness.recordEvaluation({ version, decision, reason? })` is EVALUATOR-ONLY: it requires the `harness.evaluate` capability the main agent does not hold, so the RLM can never record its own verdict (self-certification guard) — calling it denies with `lacks harness.evaluate`. `kernel.harness.promote({ version })` applies a verdict a TRUSTED source already recorded, otherwise returns `{ promote: false, reason: "…pending…" }`. `kernel.harness.versions()` lists the ledger with full hypotheses. TL;DR: propose is telemetry (never lands); promote only applies trusted verdicts; recordEvaluation is a dead-end for you BY DESIGN — do not loop on any of them.
