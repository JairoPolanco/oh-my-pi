...
- `kernel.routing.resolve({ role, taskComplexity?, … })` is ADVISORY (rule-based recommendation — the live session's actual model is decided by ModelControls, not this table). `kernel.routing.stats()` is read-only telemetry from the event log. `routing.register`/`routing.record` were REMOVED (round-11 S1: dead surfaces — nothing read the registry, and the trajectory tap feeds the event log directly).
