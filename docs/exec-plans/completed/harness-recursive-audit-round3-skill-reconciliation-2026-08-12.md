...
- Not wiring the provider-governor eviction feedback (filed — see Completion Summary follow-up debt): separate slice touching the governor constructor + event-kind semantics.
...
- 2026-08-12: **Governor eviction feedback is filed, not fixed.** Same class as round-2 F3 ("agents must not detect loss by noticing absence") but the provider-governor's per-turn whole-span evictions emit no `context.evicted` event. Wiring it needs an event sink in the governor constructor (sdk.ts:3121 has session+host) and a decision on event kind/scope + noise budget (governor eviction is the steady state under governance, not rare like the probe path). Separate slice — filed as follow-up debt.
...
- **Provider-governor eviction feedback (filed, not fixed)**: `ProviderContextGovernor`'s per-turn whole-span history eviction emits no `context.evicted` event — the real production compression path is silent while the bridge probe path (round-2 F3) reports. Same principle (never detect loss by noticing absence), different slice: thread an `onEvict` through the governor's `ContextMaterializer` (constructed at sdk.ts:3121 where session+host are available), decide event kind/scope and noise budget (governor eviction is steady-state, not rare).
